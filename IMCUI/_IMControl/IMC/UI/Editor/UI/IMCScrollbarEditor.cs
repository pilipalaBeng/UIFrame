...
{
    [CustomEditor(typeof(IMCScrollbar), true)]
    [CanEditMultipleObjects]
    public class IMCScrollbarEditor : SelectableEditor
    {
        ...
        {
            IMCScrollbar tempScrollbar = (IMCScrollbar)target;
            //EditorGUI.BeginDisabledGroup(true);
            //tempScrollbar.ControlType = (ControlType)EditorGUILayout.EnumPopup("ControlType", tempScrollbar.ControlType);
            //tempScrollbar.ContainerType = (ContainerType)EditorGUILayout.EnumPopup("ContainerType", tempScrollbar.ContainerType);
            //EditorGUI.EndDisabledGroup();
            tempScrollbar.customID = EditorGUILayout.TextField("CustomID", tempScrollbar.customID);
            base.OnInspectorGUI();
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                foreach (var item in m_HandleRect.serializedObject.targetObjects)
                {
            ...
            if (m_HandleRect.objectReferenceValue != null)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    IMCScrollbar.Direction direction = (IMCScrollbar.Direction)m_Direction.enumValueIndex;
                    foreach (var obj in serializedObject.targetObjects)
                    {
                        IMCScrollbar scrollbar = obj as IMCScrollbar;
                ...
                {
                    IMCScrollbar scrollbar = obj as IMCScrollbar;
                    IMCScrollbar.Direction dir = scrollbar.direction;
                    if (dir == IMCScrollbar.Direction.LeftToRight || dir == IMCScrollbar.Direction.RightToLeft)

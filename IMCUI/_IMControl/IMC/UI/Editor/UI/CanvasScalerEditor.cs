...
{
    [CustomEditor(typeof(IMCCanvasScaler), true)]
    ...
    public class CanvasScalerEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            bool showWorld = ((target as IMCCanvasScaler).GetComponent<Canvas>().renderMode == RenderMode.WorldSpace);
            for (int i = 0; i < targets.Length; i++)
            {
                IMCCanvasScaler scaler = targets[i] as IMCCanvasScaler;
            ...
            if (!showWorldDiffers && !(!showWorld && m_UiScaleMode.hasMultipleDifferentValues))
            {
                ...
                // Constant pixel size
                else if (m_UiScaleMode.enumValueIndex == (int)IMCCanvasScaler.ScaleMode.ConstantPixelSize)
                {
                    EditorGUILayout.PropertyField(m_ScaleFactor);
                }
                // Scale with screen size
                else if (m_UiScaleMode.enumValueIndex == (int)IMCCanvasScaler.ScaleMode.ScaleWithScreenSize)
                {
                    EditorGUILayout.PropertyField(m_ReferenceResolution);
                    EditorGUILayout.PropertyField(m_ScreenMatchMode);
                    if (m_ScreenMatchMode.enumValueIndex == (int)IMCCanvasScaler.ScreenMatchMode.MatchWidthOrHeight && !m_ScreenMatchMode.hasMultipleDifferentValues)
                    {
                ...
                else if (m_UiScaleMode.enumValueIndex == (int)IMCCanvasScaler.ScaleMode.ConstantPhysicalSize)
                {

...
{
    [CustomEditor(typeof(IMCSelectable), true)]
    public class SelectableEditor : Editor
    {
        ...
        protected virtual void OnEnable()
        {
            ...
            m_ShowColorTint.value       = (trans == IMCSelectable.Transition.ColorTint);
            m_ShowSpriteTrasition.value = (trans == IMCSelectable.Transition.SpriteSwap);
            m_ShowAnimTransition.value  = (trans == IMCSelectable.Transition.Animation);
        ...
        static IMCSelectable.Transition GetTransition(SerializedProperty transition)
        {
            return (IMCSelectable.Transition)transition.enumValueIndex;
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (graphic == null)
                graphic = (target as IMCSelectable).GetComponent<Graphic>();

            var animator = (target as IMCSelectable).GetComponent<Animator>();
            m_ShowColorTint.target = (!m_TransitionProperty.hasMultipleDifferentValues && trans == IMCButton.Transition.ColorTint);
            m_ShowSpriteTrasition.target = (!m_TransitionProperty.hasMultipleDifferentValues && trans == IMCButton.Transition.SpriteSwap);
            m_ShowAnimTransition.target = (!m_TransitionProperty.hasMultipleDifferentValues && trans == IMCButton.Transition.Animation);

            EditorGUILayout.PropertyField(m_TransitionProperty);

            ++EditorGUI.indentLevel;
            {
                if (trans == IMCSelectable.Transition.ColorTint || trans == IMCSelectable.Transition.SpriteSwap)
                {
                ...
                if (EditorGUILayout.BeginFadeGroup(m_ShowAnimTransition.faded))
                {
                    ...
                    if (animator == null || animator.runtimeAnimatorController == null)
                    {
                        ...
                        {
                            var controller = GenerateSelectableAnimatorContoller((target as IMCSelectable).animationTriggers, target as IMCSelectable);
                            if (controller != null)
                            {
                                if (animator == null)
                                    animator = (target as IMCSelectable).gameObject.AddComponent<Animator>();
        ...
        private static UnityEditor.Animations.AnimatorController GenerateSelectableAnimatorContoller(AnimationTriggers animationTriggers, IMCSelectable target)
        {
            if (target == null)
                return null;
        ...
        private static string GetSaveControllerPath(IMCSelectable target)
        {
        ...
        private static string BuildAnimationPath(IMCSelectable target)
        {
        ...
        private static void StaticOnSceneGUI(SceneView view)
        {
            ...
            for (int i = 0; i < IMCSelectable.allSelectables.Count; i++)
            {
                DrawNavigationForSelectable(IMCSelectable.allSelectables[i]);
            }
        }

        private static void DrawNavigationForSelectable(IMCSelectable sel)
        {
            if (sel == null)
                return;

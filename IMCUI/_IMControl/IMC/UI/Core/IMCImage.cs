...
namespace IMCUI.UI
{
    ...
    [AddComponentMenu("IMCUI/IMCImage", 11)]//2017年5月25日15:15:45 
    public class IMCImage : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
    {
        public enum Type
        {
            Simple,
        ...
        /// <summary>
        /// Image's texture comes from the IMCUI.Image.
        /// </summary>
        ...
        private IMCProgressBar m_progressBar;
        public IMCProgressBar progressBar
        {
            get
            {
                if (!m_progressBar)
                {
                    m_progressBar = this.GetComponent<IMCProgressBar>();
                    if (!m_progressBar)
                        m_progressBar = this.gameObject.AddComponent<IMCProgressBar>();
                }
        ...
        {
            base.Awake();
            m_controlType = ControlType.IMCImage;
            m_containerType = ContainerType.Control;
        }
        protected override void Start()
        {
            base.Start();
            if (m_isPlayOnAwakeToFrameAnimation)
                PlayFrameAnimation();
        }
        ...
        /// <summary>
        /// 保存Image的path图片保存到指定路径
        /// </summary>
        /// <param name="path"></param>
        /// <returns></returns>
        public bool SaveImageToPath(string path)
        {
            if (this.sprite != null)
            {
                string tempStr = System.IO.Path.GetExtension(path);
                byte[] buffer = null;
                if ("JPG" == tempStr.ToUpper())
                    buffer = this.sprite.texture.EncodeToJPG();
                else if ("PNG" == tempStr.ToUpper())
                    buffer = this.sprite.texture.EncodeToPNG();
                else
                    return false;//!@#$%^
                return FileSystem.Instance.WriteToFile(path, buffer, buffer.Length);
            }
            return false;
        }
        [SerializeField]
        private bool m_isNeedShowFrameAnimationPlan = false;
        [SerializeField]
        private List<Sprite> m_frameAnimationSpriteList = new List<Sprite>();
        public List<Sprite> frameAnimationSpriteList
        {
            get
            {
                return m_frameAnimationSpriteList;
            }

            set
            {
                m_frameAnimationSpriteList = value;
            }
        }

        [SerializeField]
        private float m_frameAnimationIntervalTime = 1;
        public float frameAnimationIntervalTime
        {
            get
            {
                return m_frameAnimationIntervalTime;
            }

            set
            {
                m_frameAnimationIntervalTime = value;
            }
        }

        [SerializeField]
        private bool m_isPlayOnAwakeToFrameAnimation = false;
        public bool isPlayOnAwakeToFrameAnimation
        {
            get
            {
                return m_isPlayOnAwakeToFrameAnimation;
            }

            set
            {
                m_isPlayOnAwakeToFrameAnimation = value;
            }
        }

        [SerializeField]
        private bool m_isRepeatPlayFrameAnimation = false;
        public bool isRepeatPlayFrameAnimation
        {
            get
            {
                return m_isRepeatPlayFrameAnimation;
            }

            set
            {
                m_isRepeatPlayFrameAnimation = value;
            }
        }

        [SerializeField]
        private float m_repeatPlayIntervalTime = 1;//重复播放间隔时间，播放完一遍序列帧动画后，在播放下一遍时需要时间长度

        public float repeatPlayIntervalTime
        {
            get
            {
                return m_repeatPlayIntervalTime;
            }

            set
            {
                m_repeatPlayIntervalTime = value;
            }
        }
        ...
        /// 播放帧动画
        /// </summary>
        /// <param name="intervalTime">动画间隔时间</param>
        /// <param name="isRepeatPlayAnimation">是否需要重复播放动画？</param>
        /// <param name="repeatPlayAnimationIntervalTime">重复播放间隔时间，播放完一遍序列帧动画后，在播放下一遍时需要时间长度</param>
        public void PlayFrameAnimation(float intervalTime, bool isRepeatPlayAnimation, float repeatPlayAnimationIntervalTime = 1f)
        {
            PlayFrameAnimation(0, intervalTime, isRepeatPlayAnimation, repeatPlayAnimationIntervalTime);
        }
        /// <summary>
        /// 播放帧动画
        /// </summary>
        ...
        /// <param name="isRepeatPlayAnimation">是否需要重复播放动画?</param>
        /// <param name="repeatPlayAnimationIntervalTime">重复播放间隔时间，播放完一遍序列帧动画后，在播放下一遍时需要时间长度</param>
        public void PlayFrameAnimation(int index, float intervalTime, bool isRepeatPlayAnimation, float repeatPlayAnimationIntervalTime = 1f)
        {
            if (intervalTime < 0)
            {
                Debug.LogError("intervalTime can't less than 0(tintervalTime<0),force assign is 0(intervalTime=0)");
                intervalTime = 0;
            }
            frameAnimationIntervalTime = intervalTime;

            m_isRepeatPlayFrameAnimation = isRepeatPlayAnimation;
            if (repeatPlayAnimationIntervalTime < 0)
            {
                Debug.LogError("intervalTime can't less than 0(repeatPlayAnimationIntervalTime<0),force assgin is 1(repeatPlayAnimationIntervalTime =1)");
                repeatPlayAnimationIntervalTime = 1;
            }
            m_repeatPlayIntervalTime = repeatPlayAnimationIntervalTime;

            if (index < 0)
            {
                Debug.LogError("index can't less than 0(index<0),force assign is 0(index=0)");
                index = 0;
            }
            else if (index >= m_frameAnimationSpriteList.Count)
            {
                Debug.LogError("index can't greater or equal m_frameAnimationSpriteList count,so force assign is m_frameAnimationSpriteList.Count-1(index=m_frameAnimationSpriteList.Count-1)");
                index = m_frameAnimationSpriteList.Count - 1;
            }

            PlayFrameAnimation(index);
        }
        /// <summary>
        /// 播放帧动画
        /// </summary>
        /// <param name="spriteList">精灵图list</param>
        /// <param name="index">自定义索引</param>
        /// <param name="intervalTime">动画间隔时间</param>
        /// <param name="isRepeatPlayAnimation">是否需要重复播放动画?</param>
        /// <param name="repeatPlayAnimationIntervalTime">重复播放间隔时间，播放完一遍序列帧动画后，在播放下一遍时需要时间长度</param>
        public void PlayFrameAnimation(List<Sprite> spriteList, int index, float intervalTime, bool isRepeatPlayAnimation, float repeatPlayAnimationIntervalTime = 1f)
        {
            if (spriteList.Count == 0)
            {
                Debug.LogError("spriteList count can't equal is 0(spriteList.Count==0),so return function");
                return;
            }
            else
            {
                m_frameAnimationSpriteList.Clear();
                m_frameAnimationSpriteList = spriteList;
            }
            PlayFrameAnimation(index, intervalTime, isRepeatPlayAnimation, repeatPlayAnimationIntervalTime);
        }
        private int m_frameAnimationIndex = 0;
        public void PlayFrameAnimation(int index = 0)
        {
            InitializeFrameAnimationPrepare(index);
            Invoke("PlayAnimation", m_frameAnimationIntervalTime);
        }
        private void InitializeFrameAnimationPrepare(int indexNumb = 0)
        {
            m_frameAnimationIndex = indexNumb;
            FrameAnimationToggleSprite(m_frameAnimationIndex);
        }
        private void FrameAnimationToggleSprite(int indexNumb)
        {
            this.sprite = m_frameAnimationSpriteList[indexNumb];
        }
        private bool? isPlayAnimation = null;
        public void PlayAnimation()
        {
            if (isPlayAnimation == null || isPlayAnimation == false)
                isPlayAnimation = true;
            m_frameAnimationIndex++;
            if (m_frameAnimationIndex < m_frameAnimationSpriteList.Count)
            {
                FrameAnimationToggleSprite(m_frameAnimationIndex);
                Invoke("PlayAnimation", m_frameAnimationIntervalTime);
            }
            ...
            {
                if (m_isRepeatPlayFrameAnimation)
                {
                    m_frameAnimationIndex = -1;
                    Invoke("PlayAnimation", m_repeatPlayIntervalTime);
                }
            }
        }
        public void StopAnimation()
        {
            ...
            FrameAnimationToggleSprite(0);
            m_frameAnimationIndex = 0;
            isPlayAnimation = null;
        }
